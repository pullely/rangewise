...
Your backend holds one Rangewise API key and **zero GitHub credentials**.
...
const rangewise = new Rangewise({
  baseUrl: process.env.RANGEWISE_API_URL!,
  token: process.env.RANGEWISE_API_KEY!, // service-principal key
});

// Mint a token scoped to exactly the repos + permissions you need.
// TTL ≤ 1h; mint per task and let it expire — never store it.
const { token } = await rangewise.integrations.issueGithubToken(ORG_ID, {
