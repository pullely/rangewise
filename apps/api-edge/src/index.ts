...
import { isRangeRoute, handleRangeRoute } from "./range-facade";
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    } else if (isRangeRoute(url.pathname)) {
      response = await handleRangeRoute(request, env, requestId, url.pathname);
