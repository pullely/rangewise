...
// deleted; all wire I/O now goes through the typed `Rangewise` resource
...
//   - `createClient` — constructs a `Rangewise` against a target, wired
...
//     `Promise<T>` + thrown `RangewiseError` into the same shape the
...
import { Rangewise, RangewiseError, type ClientOptions } from "@saas/sdk";
import { apiEdgeWorkersDevUrl } from "./app-config";

export type ApiClient = Rangewise;
...
export function createClient(target: ApiTarget, token: string | null): Rangewise {
  const opts: ClientOptions = { baseUrl: target.url };
  if (token) opts.auth = { kind: "bearer", token };
  return new Rangewise(opts);
}

/**
 * Adapt an SDK promise into the `ApiResult<T>` envelope. Typed
 * `RangewiseError` subclasses surface as `{ ok: false }` carrying the
...
export async function wrap<T>(fn: () => Promise<T>): Promise<ApiResult<T>> {
  try {
    ...
    if (e instanceof RangewiseError) {
