...
describe("resolveConfigDir", () => {
  it("respects RANGEWISE_CONFIG_DIR override", () => {
    const prev = process.env["RANGEWISE_CONFIG_DIR"];
    process.env["RANGEWISE_CONFIG_DIR"] = "/tmp/cli-override";
    try {
      expect(resolveConfigDir()).toBe("/tmp/cli-override");
    } finally {
      if (prev === undefined) delete process.env["RANGEWISE_CONFIG_DIR"];
      else process.env["RANGEWISE_CONFIG_DIR"] = prev;
    }
  });
});
