// CLI error helpers. Translate `RangewiseError` subclasses (from
...
export class MissingOrgContextError extends Error {
  constructor() {
    super("no active organization (run `rangewise org use <id>`)");
...
export function formatCliError({ err, mode }: FormatErrorInput): FormattedError {
  ...
  if (err instanceof UnauthenticatedError) {
    return formatSdk(4, err, "token rejected — run `rangewise login` to refresh", mode);
  }
  if (err instanceof RangewiseError) {
