...
`rangewise` — first-class TypeScript CLI for the Rangewise control
...
This package is internal to the `rangewise` monorepo and is not
...
```
rangewise login    [--api-url=URL] [--token=BEARER]
rangewise logout
rangewise whoami
```
...
```
rangewise org list
rangewise org use <org-id>
rangewise org members
rangewise project list
```

Writes (Task 0101):

```
rangewise org invite <email> [--role=ROLE] [--idempotency-key=KEY] [--org=ORG_ID]
rangewise project create <name> [--idempotency-key=KEY]
rangewise env create <project-id> <name> [--idempotency-key=KEY]
rangewise api-key create <name> [--scope=SCOPE] [--idempotency-key=KEY]
rangewise webhook create <url> [--event=EVENT[,EVENT2,...]] [--idempotency-key=KEY]
```

Cross-resource reads (Task 0101):

```
rangewise usage summary    [--metric=METRIC] [--from=ISO] [--to=ISO]
rangewise billing summary
rangewise audit list       [--limit=N] [--cursor=CURSOR] [--category=CAT] [--all]
```
...
context (`rangewise org use <org-id>`). Only `org invite` accepts an
...
The shipped V1 is **token-paste**: `rangewise login` prompts for a
...
- `FileTokenStore` fallback: `~/.config/rangewise/credentials.json`,
  mode **0600**, parent directory mode **0700**.

Active organization context lives at
`~/.config/rangewise/config.json` (mode 0644, not a secret). Override
both via `RANGEWISE_CONFIG_DIR` (used by tests).
