// Base HTTP transport for the Rangewise SDK.
//
// Goals:
...
// - Typed errors: every non-2xx response is decoded into a `RangewiseError`
...
export interface ClientOptions {
  /**
   * Base URL of the Rangewise api-edge worker, e.g.
   * `https://api.rangewise.app`. Trailing slash is stripped.
   */
  baseUrl: string;
 ...
 * `Rangewise` (the client class wired up in `index.ts`).
 */
export class Transport {
  ...
  constructor(options: ClientOptions) {
    ...
    if (typeof fetchImpl !== "function") {
      throw new TypeError(
        "Rangewise SDK requires a fetch implementation. Pass `fetch` in options on platforms without a global.",
      );
    }
...
export function generateRequestId(): string {
  ...
  throw new Error(
    "Rangewise SDK requires the Web Crypto API (globalThis.crypto). No source of randomness was found.",
  );
}
