...
import { Rangewise } from "../index.js";
import {
  BadRequestError,
  ConflictError,
  ForbiddenError,
...
describe("Transport — success path", () => {
  it("unwraps the data envelope on GET", async () => {
    ...
    const client = new Rangewise({ baseUrl: "https://api.test", fetch });
...
describe("Error decoding — typed branches", () => {
  ...
  const cases: Array<[string, number, new (...args: never[]) => RangewiseError]> = [
  ...
  it("forward-compat: unknown error code → base RangewiseError", async () => {
    ...
    try {
      ...
      expect(err).toBeInstanceOf(RangewiseError);
      expect(err).not.toBeInstanceOf(BadRequestError);
      expect((err as RangewiseError).code).toBe("future_quota_exceeded");
    }
  });
