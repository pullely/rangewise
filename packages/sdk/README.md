...
Rangewise TypeScript SDK — typed, dependency-free, runtime-agnostic client
for the Rangewise control plane API.

## Install (workspace)

This package is internal to the `rangewise` monorepo. Add it as a
...
```ts
import { Rangewise } from "@saas/sdk";

const client = new Rangewise({
  baseUrl: "https://api.rangewise.app",
  auth: { kind: "bearer", token: process.env["RANGEWISE_TOKEN"]! },
});
...
All non-2xx responses throw a typed subclass of `RangewiseError`. Branch on
...
| `RangewiseError` (base)   | _any_                 | _any_          |

Unknown error codes (forward-compatible — e.g. a future `quota_exceeded`)
decode to the base `RangewiseError` carrying the raw envelope. Non-JSON 5xx
...
A custom `fetch` implementation can be injected via `new Rangewise({ fetch })`.
